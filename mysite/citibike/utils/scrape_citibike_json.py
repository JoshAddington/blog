...
def scrape_json():
    ...
    response = requests.get('http://www.citibikenyc.com/stations/json')
    json_response = response.json()
    station_info = json_response['stationBeanList']
    # postgres needs 24 hour time, citibike supplies 12 w/ am/pm
    execution_time = datetime.datetime.strptime(json_response['executionTime'],
                                                "%Y-%m-%d %I:%M:%S %p"
                                                ).strftime("%Y-%m-%d %H:%M:%S")
    # split datetime into date and time
    date = execution_time[0:10]
    time = execution_time[11:]
    result = update_table(station_info, date, time)
    return ("%i stations updated." % result)
